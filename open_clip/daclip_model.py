...
class DaCLIP(nn.Module):
    ...
    def forward(
        ...
        (caption, degradation_main,degradation_sub) = text.chunk(3, dim=-1) if text is not None else (None, None)
        ...
        text_degra_features = self.encode_text(degradation_main, normalize=True) if degradation_main is not None else None
        text_degra_sub_features = self.encode_text(degradation_sub,
                                               normalize=True) if degradation_main is not None else None
        return {
            "image_features": image_features,
            "text_features": text_features,
            "image_degra_features": image_degra_features,
            "text_degra_features": text_degra_features,
            "text_degra_sub_features":text_degra_sub_features,
